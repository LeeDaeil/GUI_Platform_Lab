...
from CNS_Monitoring_module.PARKJIHUN.Plot_module_ver2 import Plot_module

class Diagnosis_module:
    def __init__(self):
        self.plot_module = Plot_module()

    def diagnosis(self, time, row, check_data, check_parameter, threshold_train_untrain,
                  threshold_normal_abnormal,
                  train_untrain_reconstruction_error,
                  normal_abnormal_reconstruction_error,
                  abnormal_procedure_result,
                  shap_values,
        ...
        if train_untrain_reconstruction_error[row-9] <= threshold_train_untrain: # Trained Data
            ...
            if normal_abnormal_reconstruction_error[row-9] > threshold_normal_abnormal: # Abnormal Data
                ...
                self.plot_module.SubPlotting_abnormal(time,
                                                      row,
                                                      train_untrain_reconstruction_error,
                                                      normal_abnormal_reconstruction_error,
                                                      check_data,
                                                      check_parameter,
                                                      threshold_train_untrain,
                                                      threshold_normal_abnormal,
                                                      abnormal_procedure_result,
                                                      shap_values,
                                                      abnormal_verif_reconstruction_error,
                ...
                self.plot_module.SubPlotting_normal(time, row,
                                                    train_untrain_reconstruction_error,
                                                    normal_abnormal_reconstruction_error,
                                                    threshold_train_untrain,
                                                    threshold_normal_abnormal)
            ...
            self.plot_module.SubPlotting_untrain(time, row, train_untrain_reconstruction_error,
                                                 threshold_train_untrain )
        return